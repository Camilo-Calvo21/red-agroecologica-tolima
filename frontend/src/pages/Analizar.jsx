...
export default function Analizar() {
  ...
  const cargarFincas = async () => {
    try {
      ...
      setFincas(data)
      // Si no había finca seleccionada y solo hay una, la pre-selecciona
      if (!fincaId && data.length === 1) {
        setFincaId(data[0].id)
      }
    } catch (e) {
      setError('No se pudieron cargar tus fincas: ' + e.message)
    } finally {
      setCargando(false)
    }
  }

  const puedeAnalizar = imagen && fincaId && !procesando
  ...
  const onAnalizar = async () => {
    ...
    setResultado(null)
    try {
      const muestra = await muestrasAPI.crear({
        fincaId,
        imagen,
        ph,
        humedad,
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12 pb-8">
        ...
        >
          ...
          <h1 className="font-display text-4xl text-tierra-900 mb-2">
            Analizar <span className="font-display-italic text-musgo-700">cromatograma</span>
          </h1>
          <p className="text-tierra-600 max-w-2xl">
            Sube la fotografía del cromatograma de Pfeiffer, ajusta los valores
            de pH y humedad medidos, y obtén la imagen con el anillo orgánico.
          </p>
        </motion.div>
      </div>
      ...
      <main className="max-w-7xl mx-auto w-full px-6 pb-12">
        <div className="grid lg:grid-cols-2 gap-7" id="formulario">
          ...
          <motion.div
            ...
            className="card-elevated p-7 space-y-7"
          >
            <div>
              <span className="badge bg-tierra-100 text-tierra-700 mb-3">
                Paso 1 — Datos
              </span>
              <h3 className="font-display text-2xl text-tierra-900 leading-tight">
                Cuéntale al sistema sobre tu suelo
              </h3>
            </div>
            ...
            <CargaImagen
              imagen={imagen}
              onImagenSeleccionada={setImagen}
              deshabilitado={procesando}
            />
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Observaciones de campo, condiciones del muestreo..."
              />
            </div>
            ...
            <div className="pt-2">
              ...
              {error && (
                <p className="text-xs text-ambar-600 text-center mt-3 font-medium">
                  ⚠ {error}
                </p>
              )}

              {!puedeAnalizar && !procesando && !error && (
                <p className="text-xs text-tierra-500 text-center mt-3">
                  {!fincaId && 'Selecciona una finca · '}
                  {!imagen && 'Sube una imagen · '}
                  {fincaId && imagen && 'Listo para analizar'}
                </p>
              )}
            </div>
          </motion.div>
