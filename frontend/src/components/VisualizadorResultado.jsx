...
/**
 * Visualizador de resultados con tres modos.
 * Textos actualizados: bio retrato, cromatografía ampliada, generar.
 */
export default function VisualizadorResultado({ muestra, procesando, onReiniciar }) {
  ...
  if (!muestra) {
    return (
      <div className="card-elevated p-8 min-h-[600px] flex flex-col items-center justify-center text-center">
        ...
        <p className="text-2xs uppercase tracking-wider text-tierra-500 font-semibold mb-3">
          AQUÍ APARECERÁ TU CROMATOGRAFÍA AMPLIADA
        </p>
        <h3 className="font-display text-2xl text-tierra-700 mb-2">
          Aquí aparecerá tu cromatografía ampliada
        </h3>
        <p className="text-tierra-500 text-sm max-w-sm">
          Una vez generes el bio retrato, podrás compararlo con la imagen original,
  ...
  const descargar = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `bio_retrato_pH${muestra.ph.toFixed(1)}_H${muestra.humedad.toFixed(0)}.png`
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        {modo === 'lado-a-lado' && (
          <motion.div key="lado" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid sm:grid-cols-2 gap-4">
            <ImagenEtiqueta src={muestra.imagen_original_url} etiqueta="Original" sub="Sin procesar" />
            <ImagenEtiqueta src={muestra.imagen_procesada_url} etiqueta="Con bio retrato" sub="Procesada" destacado />
          </motion.div>
        )}

        {modo === 'comparar' && tieneOriginal && tieneProcesada && (
          <motion.div key="slider" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
            <div className="relative aspect-square rounded-xl overflow-hidden border border-tierra-200">
              ...
              <div className="absolute inset-0 overflow-hidden" style={{ width: `${posicion}%` }}>
                <img src={muestra.imagen_procesada_url} alt="Con bio retrato" className="absolute inset-0 h-full object-cover" style={{ width: `${100 / (posicion / 100)}%` }} />
              </div>
              <div className="absolute inset-y-0 w-0.5 bg-white shadow-lg pointer-events-none" style={{ left: `${posicion}%` }}>
              ...
              <div className="absolute top-3 right-3 badge bg-musgo-700/90 text-white backdrop-blur-sm">Con bio retrato</div>
            </div>
            <input
              type="range" min={0} max={100} value={posicion}
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={descargar} disabled={descargando || !tieneProcesada} className="btn-primary flex-1">
          {descargando ? (
        ...
        <button onClick={onReiniciar} className="btn-secondary">
          <RotateCcw className="w-4 h-4" /> Nueva muestra
        </button>
      </div>

      {errorDescarga && (
        <p className="text-xs text-ambar-600 text-center font-medium mt-2">⚠ {errorDescarga}</p>
      )}
    </motion.div>
  )
}
...
function ImagenEtiqueta({ src, etiqueta, sub, destacado, grande }) {
  ...
  return (
    <div>
      <div className={`relative rounded-xl overflow-hidden border ${destacado ? 'border-musgo-300 shadow-lg shadow-musgo-900/5' : 'border-tierra-200'}`}>
        {src && !errorCarga ? (
          <img src={src} alt={etiqueta} onError={() => setErrorCarga(true)} className={`w-full object-cover ${grande ? 'aspect-square max-h-[600px]' : 'aspect-square'}`} />
        ) : (
          <div className="aspect-square bg-tierra-100 flex flex-col items-center justify-center p-4 text-center">
            ...
            <p className="text-xs text-tierra-500">{errorCarga ? 'No se pudo cargar la imagen' : 'Sin imagen'}</p>
          </div>
        )}
      </div>
      <div className="mt-2.5 px-1">
        <div className={`text-2xs uppercase tracking-wider font-semibold ${destacado ? 'text-musgo-700' : 'text-tierra-500'}`}>{etiqueta}</div>
